...
- `--months N` (1–12, default 1) emits a recurring monthly series per customer: stable VS across the series, month-suffixed invoice numbers (`2026-0001-01`…`-06`), constant fee, final month `unpaid` (no transaction; statement marks it `Unpaid · no_payment_found`), earlier months `exact_single`. Annual extras are skipped in this mode. `--months 1` is byte-identical to the prior behavior; all multi-month logic is gated behind `months > 1`. New `unpaid` payment scenario is handled in both scripts.
